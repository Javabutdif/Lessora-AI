...
Run `npm run workflow -- doctor` before starting any task to verify the workflow layer is installed. Key commands:

- `npm run workflow -- scaffold --slug <topic>` — create a dated task brief, spec, and plan
- `npm run workflow -- check` — validate the active task brief
- `npm run workflow -- pack [--compress] [--to <tool>]` — write a handoff pack to `docs/ai/handoffs/`
- `npm run workflow -- finalize` — archive completed task and linked artifacts
- `npm run workflow -- skill add <repo>` — install agentskills.io skills from a remote repo
...
- run `npm run workflow -- doctor && npm run workflow -- check` before finishing
