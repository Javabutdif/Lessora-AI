...
- `skills/`: seven skills (`workflow`, `docs`, `scripts`, `subagents`, `react-native-architecture`, `@tank/node-express`, `react-native-design`) with agentskills.io-compliant `SKILL.md` frontmatter; install into any compatible tool via `npx skills add Nimblco/nimblco` or `npm run workflow -- skill add Nimblco/nimblco`
